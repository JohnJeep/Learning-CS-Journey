 ...
 * @LastEditTime: 2021-06-30 20:49:31
...
- [4. Network layer(网络层)](#4-network-layer网络层)
	- [4.1. complement (补码)](#41-complement-补码)
	- [4.2. Subnet mask(子网掩码)](#42-subnet-mask子网掩码)
	- [4.3. default route (默认路由)](#43-default-route-默认路由)
	- [4.4. IP( Internet Protocol)](#44-ip-internet-protocol)
		- [4.4.1. IP记录路由选项](#441-ip记录路由选项)
		- [4.4.2. IP源站选路选项](#442-ip源站选路选项)
		- [4.4.3. IP选路](#443-ip选路)
	- [4.5. ARP地址解析协议](#45-arp地址解析协议)
	- [4.6. RARP逆地址解析协议](#46-rarp逆地址解析协议)
	- [4.7. ICMP(Internet控制报文协议)](#47-icmpinternet控制报文协议)
	- [4.8. ping](#48-ping)
	- [4.9. Traceroute](#49-traceroute)
...
> 优化云服务器性能：TCP splitting (TCP 分岔)。若果终端系统距离数据中心很远，导致 RTT 很大，这是因为慢启动会导致响应时间性能很差。
...
- User Datagram Protocol (用户数据报协议) UDP 是一种简洁、轻量级的传输协议，提供最少的服务。UDP是无连接的 (connectionless)，在两个进程通信前没有握手过程。 UDP 协议提供一种不可靠数据传输服务，也就是说，当一个进程发送由一个报文 (message) 进UDP套接字时，UDP协议并不保证该 message 会到达接收进程。不仅如此，到达接收进程的报文也可能是乱序到达的。
...
- ARP的 packet 格式
  <p align="center"><img src="./figure/ARP的 packet 格式.png"></p>
