...
- **QoS（服务质量）**
  ...
  **常见且重要的QoS策略：**
  ...
  | **Deadline**      | 指定数据更新的最大间隔，超过则触发回调（确保数据时效性） | 用于**时序监控**。如果发布者未在指定周期内发布数据，会触发回调。 |
  | **Lifespan**      | 数据在发布后的“有效期”，过期后会被丢弃。                 | 用于传输有时效性的数据。                                     |
