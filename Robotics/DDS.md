...
DDS的底层通信协议是**RTPS**。RTPS是DDS的 Wire Protocol，它定义了数据如何在网络上传输。
...
FastDDS 是 eProsima 公司开发的一个高性能、开源的 DDS 实现，曾经是 ROS 2 的默认 DDS 实现。它提供了丰富的功能和工具支持，适用于各种实时通信场景。



# 5. CycloneDDS

CycloneDDS 是 Eclipse 基金会开发的一个轻量级、高性能的 DDS 实现，目前是 ROS 2 推荐的默认 DDS 实现。它以其高效的性能和良好的稳定性而受到广泛使用，特别适合资源受限的环境。
