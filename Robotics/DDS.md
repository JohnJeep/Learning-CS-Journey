 ...
 * @LastEditTime: 2026-07-05 14:53:42
...
指定环境变量表，将其它的 `rmw` 切换为  `rmw_cyclonedds`
...
- **RouDi 守护进程**：Iceoryx 需要一个中心化的守护进程（RouDi: Routing and Discovery）在系统启动时运行，负责协调共享内存的分配，有点像一个轻量级的资源管理器。
...
**iceoryx（classic）**：采用中心化架构。整个系统分三层——RouDi 守护进程（中心协调者）、应用进程（发布者/订阅者/客户端/服务端）、共享内存段（数据存储）。通信是双平面架构：控制平面走 IPC（Unix Domain Socket 或命名管道），应用通过它向 RouDi 注册、创建端口、做服务发现；数据平面走共享内存，实际数据传输是应用之间直接进行的零拷贝，不经过 RouDi。也就是说，虽然实际"搬数据"不经过 RouDi，但每个应用启动时都必须先跟 RouDi 打交道——每个 iceoryx 应用都必须初始化一个 PoshRuntime 实例，用它来把共享内存段映射进本进程地址空间，并和 RouDi 建立 IPC 通信。RouDi 名字本身就说明了它的职责：RouDi 代表 Routing and Discovery（路由与发现）。
...
3. ROS2 层面的整合 ：在 ROS2 中，这个组合通常表现为：
...
iceoryx2 是 Eclipse 基金会另起了一个新项目 **iceoryx2**，用 Rust 从零构建。iceoryx2 是一个**零拷贝、无锁的进程间通信（IPC）中间件**，由 Eclipse 基金会维护，是老牌 C++ 项目 **iceoryx**（也叫 iceoryx classic）的 Rust 继任者。

作为 iceoryx 的"下一代继任者"，与 iceoryx 两者是并行维护的独立项目（iceoryx classic 目前处于维护模式，官方推荐新项目用 iceoryx2）。


### 6.6.1. 它是干什么的

它让运行在同一台机器上的不同进程能够高效地交换大量数据，而不需要把数据拷贝来拷贝去。传统的 IPC 方式（比如 socket、管道）通常要经过"发送方内存 → 内核 → 接收方内存"这样的拷贝路径；iceoryx2 的做法是把数据直接放在共享内存里，进程之间传递的只是"指向这块内存的引用"，数据本身不动。发布者从共享内存里借出一块区域，直接把消息写进去，只写一次；订阅者原地读取，增加订阅者也只是共享更多引用，而不是拷贝更多数据。这样带来的好处是**延迟低、且延迟不随数据量增大而明显增长**，特别适合传输大块数据（比如传感器图像、点云）。


### 6.6.2. 支持的通信模式

支持发布订阅（pub/sub）、请求响应（request/response）、pipeline（规划中）以及 blackboard 模式，还支持通过事件（events）在进程间做快速可靠的信号通知

### 6.6.3. 和 iceoryx classic 的关系/区别

iceoryx2 的血统可以追溯到 Eclipse iceoryx 项目，主要是为了解决一个老大难问题——中心化 daemon。iceoryx classic 需要一个叫 RouDi 的中心守护进程来管理共享内存和服务发现，而 iceoryx2 采用完全去中心化的架构，彻底不需要中心daemon了——每个服务通过唯一的 ServiceName 来标识，进程间通信只需先创建/打开一个 service（相当于一个 port factory），然后从这个 factory 创建发布者/订阅者等端点即可通信。

### 6.6.4. 典型应用场景

它最初的应用背景是汽车行业和自动驾驶（大量传感器数据要在不同进程/模块间高速传输），但同样适用于机器人、游戏开发等对延迟和吞吐量敏感的场景——这个和你之前聊的机器人中间件（rosa）方向也算是相关领域。

### 6.6.5. 总结对比表

| 维度          | iceoryx（classic）                                        | iceoryx2                                                     |
| ------------- | --------------------------------------------------------- | ------------------------------------------------------------ |
| 实现语言      | C++                                                       | Rust（核心），提供 C/C++/Python/Rust 绑定                    |
| 架构          | 中心化（依赖 RouDi daemon）                               | 完全去中心化，无 broker                                      |
| 性能          | 基线                                                      | 最高快 10 倍                                                 |
| 内存池配置    | 编译期固定                                                | 运行时动态可调                                               |
| 消息模式      | 主要是 pub/sub                                            | pub/sub、event、request-response、blackboard，pipeline 规划中 |
| 容错/健康监测 | 有限                                                      | 内建健康监测、WaitSet、deadline/interval                     |
| 调试工具      | 内省客户端                                                | iox2 CLI                                                     |
| 跨主机通信    | 需搭配 DDS 网关                                           | 内建网络隧道                                                 |
| ROS2 集成     | 通过 CycloneDDS 集成，功能有限（不支持 services/actions） | 独立 RMW 绑定，目标是安全认证级                              |
| 当前状态      | 维护模式                                                  | 官方推荐、积极开发                                           |
...
- https://deepwiki.com/eclipse-iceoryx/iceoryx
