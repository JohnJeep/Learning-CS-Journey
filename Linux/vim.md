...
- `^`	  移动光标至本行第一个非空字符处
- `w(word)`	  向前（往右）移动一个词，光标停在下一个单词的开头。注意：单词与单词中间有空格时，会跳过空格，到下一个单词的开头。例如：`5w`  向前移动五个词
- `W(大写)` 移动到下一个单词开头，但忽略一些标点符号。
- `b(back)`   从当前光标处 `向后(向左)` 跳到每个单词的开头，遇见 `,` 会停止。例如： `5b`  向后移动五个词
- `B`	  从当前光标处 `向后(向左)` 跳到每个单词的开头，只认以空格分隔的词。
- `e(end of word)`   从当前光标处 `向前(向右)` 跳到每个单词的末尾，只认以空格分隔的词。
