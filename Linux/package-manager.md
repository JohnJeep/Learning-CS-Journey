...
**apt源**：集中存储deb包的服务器，通常以http、nfs、ftp、file等协议提供deb包下载安装服务。互联网中的apt源一般分为发行方和第三方提供。Ubuntu默认使用发行方apt源。安装软件时会自动解决依赖关系。
...
- `dpkg -i` 手动安装软件包(这个命令并不能解决软件包之前的依赖性问题)，如果在安装某一个软件包的时候遇到了软件依- 赖的问题,可以用 `apt-get -f install` 在解决信赖性这个问题。
