...
- 嵌入式单片机中，包含有外围设备的变量每个字母大写，后面加`_`，下划线后面每个单词的首字母大写，结构体结尾添加 `_TypeDef`，
...
- #define
  ...
  - #define宏，的名字全部都为大写。不要出现大小写混杂的形式。
    ...
    ```
- #include
  - C++代码使用 `#include` 来引入其它的模块的头文件。尽可能，按照模块的稳定性顺序来排列 `#include` 的顺序，按照稳定性从高到低排列。


# 12. OOP规约(Object Oriented Programming) 
