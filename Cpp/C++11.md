 ...
 * @LastEditTime: 2025-11-13 20:31:18
...
### 1.3.1. 背景与问题：为什么需要 nullptr？

#### 1.3.1.1. 1.1 C++98/03 的问题：NULL 的缺陷
...
#### 1.3.4.3. 4.3 用户定义类型的 nullptr 支持
...
1. lambda 没有默认的构造和赋值操作函数(`default constructor` 和 `assignment operator`)。
2. 捕获变量中没有加 `=` 或 `&` 时，编译器默认按照是值传递 `=`；在捕获变量时，一般建议使用 **传引用，而非传值**。
