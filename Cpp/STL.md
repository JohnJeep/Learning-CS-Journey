 ...
 * @LastEditTime: 2026-03-06 14:22:40
...
从语言实现的层面分析，Algorithm 采用 function template 实现的，而 Container、Iterator、Functor、Adaptor、Allocator 都是采用 class template 实现的。
...
STL 标准库中 容器内存储的元素都必须能够拷贝，而 C++ 编译器默认提供的是浅拷贝，程序在执行时，则会出现问题。因此需要 **重写构造函数** 和重载 **=** 操作运算符，执行深拷贝。
...
Array会把元素复制到其内部的 static C-style array 中。这些元素总是拥有一个明确次序。
...
如果你需要一个有固定元素量的序列，`classarray<>` 将带来最效能，因为内存被分配下 stack中（如果可能的话），
