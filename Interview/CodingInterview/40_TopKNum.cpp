 ...
 * @LastEditTime: 2021-01-13 23:35:36
 ...
 *                   法二：1、当原数组中的数据顺序不可修改，且K数值很大时（海量数据），
 *                            采用快速排序可能会占满内存，效率不高。因此使用最大堆数据结构实现
 *                         2、先把前 k 个数据建立一个大根堆，然后对后面的 length-k 个数依次遍历，
