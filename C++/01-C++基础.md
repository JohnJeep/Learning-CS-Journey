 ...
 * @LastEditTime: 2021-05-18 16:19:51
...
- [2. StyleGuide(规范)](#2-styleguide规范)
- [3. Concept(概念)](#3-concept概念)
- [4. Namespace(命名空间)](#4-namespace命名空间)
- [5. Header file(头文件)](#5-header-file头文件)
- [6. Reference(引用)](#6-reference引用)
  - [6.1. 普通引用](#61-普通引用)
  - [6.2. lvalue reference(左值引用)](#62-lvalue-reference左值引用)
  - [6.3. rvalue reference(右值引用)](#63-rvalue-reference右值引用)
  - [6.4. const reference(常量引用)](#64-const-reference常量引用)
...
- [8. constructor(构造函数)](#8-constructor构造函数)
  - [8.1. copy constructor(拷贝构造)](#81-copy-constructor拷贝构造)
  - [8.2. copy assignment operator(拷贝赋值)](#82-copy-assignment-operator拷贝赋值)
- [9. destructor(析构函数)](#9-destructor析构函数)
- [10. empty class](#10-empty-class)
- [11. class object && class pointer(类对象与类指针的区别)](#11-class-object--class-pointer类对象与类指针的区别)
- [12. new && delete](#12-new--delete)
- [13. overloading(重载)](#13-overloading重载)
- [14. overwrite(重写)](#14-overwrite重写)
- [15. const](#15-const)
- [16. static](#16-static)
- [17. this](#17-this)
- [18. mutable](#18-mutable)
- [19. pass by value && pass by reference(参数传递与返回值传递)](#19-pass-by-value--pass-by-reference参数传递与返回值传递)
- [20. friend(友元)](#20-friend友元)
- [21. operator overloading(操作运算符重载)](#21-operator-overloading操作运算符重载)
- [22. C++典型问题](#22-c典型问题)
  - [22.1. `.h`与 `.hpp` 文件区别](#221-h与-hpp-文件区别)
  - [22.2. ++i与i++效率问题](#222-i与i效率问题)
  - [22.3. 传值、传引用、传指针三者的区别](#223-传值传引用传指针三者的区别)
- [23. smart pointer(智能指针)](#23-smart-pointer智能指针)
  - [23.1. 为什么要使用智能指针？](#231-为什么要使用智能指针)
  - [23.2. 智能指针原理](#232-智能指针原理)
  - [23.3. 使用智能指针的优点](#233-使用智能指针的优点)
  - [23.4. auto_ptr](#234-auto_ptr)
  - [23.5. unique_ptr](#235-unique_ptr)
    - [23.5.1. 概述](#2351-概述)
    - [23.5.2. 为什么要用 unique_ptr](#2352-为什么要用-unique_ptr)
    - [23.5.3. <font color=red> 注意点</font>](#2353-font-colorred-注意点font)
    - [23.5.4. 成员函数](#2354-成员函数)
  - [23.6. shared_ptr](#236-shared_ptr)
    - [23.6.1. 概述](#2361-概述)
    - [23.6.2. 为什么要使用 shared_ptr](#2362-为什么要使用-shared_ptr)
    - [23.6.3. 成员函数](#2363-成员函数)
    - [23.6.4. 底层原理](#2364-底层原理)
    - [23.6.5. 注意点](#2365-注意点)
  - [23.7. weak_ptr](#237-weak_ptr)
    - [23.7.1. 概述](#2371-概述)
    - [23.7.2. 为什么要使用 weak_ptr](#2372-为什么要使用-weak_ptr)
    - [23.7.3. 底层原理](#2373-底层原理)
    - [23.7.4. 注意点](#2374-注意点)
  - [23.8. 参考](#238-参考)
- [24. typename](#24-typename)
- [25. Explicit Conversions(显示类型转换)](#25-explicit-conversions显示类型转换)
- [26. Exception(异常处理)](#26-exception异常处理)
...
## 8.2. copy assignment operator(拷贝赋值) 
  ```C++
...
# 11. class object && class pointer(类对象与类指针的区别)
```C++
class Stu
{
private:
...
# 19. pass by value && pass by reference(参数传递与返回值传递)
...
# 21. operator overloading(操作运算符重载)
- 为什么要操作符重载？
...
## 23.5. unique_ptr
### 23.5.1. 概述
`unique_ptr` 实现的是一种独一无二拥有权 (exculsive ownership) 的概念。保证一个对象和其相应资源同一时间内只能被一个 smart 指针拥有(ownership)。当 unique_ptr 被销毁时，它所指向的对象也就自动销毁。

### 23.5.2. 为什么要用 unique_ptr
它对于避免资源泄露(例如以new创建对象后因为发生异常而忘记调用delete)特别有用。 


### 23.5.3. <font color=red> 注意点</font>
...
## 23.6. shared_ptr
### 23.6.1. 概述
`share_ptr` 实现的是一种共享所有权 (shared ownership)的概念。多个 smart 指针可以指向同一个对象，该对象和它的相关资源会在最后一个指针的指向 `reference` 被销毁时，得到释放。

### 23.6.2. 为什么要使用 shared_ptr
...
# 25. Explicit Conversions(显示类型转换)
C++中的类型转换有4种。
