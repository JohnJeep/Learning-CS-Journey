...
- [1. C++ Primer(C++基础)](#1-c-primerc基础)
  ...
  - [1.22. smart pointer(智能指针)](#122-smart-pointer智能指针)
    ...
    - [1.22.2. unique_ptr](#1222-unique_ptr)
      - [1.22.2.1. 概述](#12221-概述)
      - [1.22.2.2. 为什么要用 unique_ptr](#12222-为什么要用-unique_ptr)
      - [1.22.2.3. <font color=red> 注意点</font>](#12223-font-colorred-注意点font)
      - [1.22.2.4. 成员函数](#12224-成员函数)
    - [1.22.3. shared_ptr](#1223-shared_ptr)
      - [1.22.3.1. 概述](#12231-概述)
      - [1.22.3.2. 为什么要使用 shared_ptr](#12232-为什么要使用-shared_ptr)
      - [1.22.3.3. 成员函数](#12233-成员函数)
      - [1.22.3.4. 底层原理](#12234-底层原理)
      - [1.22.3.5. 注意点](#12235-注意点)
    - [1.22.4. weak_ptr](#1224-weak_ptr)
      - [1.22.4.1. 概述](#12241-概述)
      - [1.22.4.2. 为什么要使用 weak_ptr](#12242-为什么要使用-weak_ptr)
      - [1.22.4.3. 底层原理](#12243-底层原理)
      - [1.22.4.4. 注意点](#12244-注意点)
...
### 1.22.2. unique_ptr
#### 1.22.2.1. 概述
`unique_ptr` 是保证一个对象和其相应资源同一时间内只能被一个指针拥有(ownership)。当 unique_ptr 被销毁时，它所指向的对象也就自动销毁。

#### 1.22.2.2. 为什么要用 unique_ptr
它对于避免资源泄露(例如以new创建对象后因为发生异常而忘记调用delete)特别有用。 


#### 1.22.2.3. <font color=red> 注意点</font>
- unique_ptr 不允许执行 copy 和 assignment 操作。
...
### 1.22.3. shared_ptr
#### 1.22.3.1. 概述
`share_ptr` 多个指针指向同一个对象。

#### 1.22.3.2. 为什么要使用 shared_ptr
...
### 1.22.4. weak_ptr 
#### 1.22.4.1. 概述
`weak_ptr` 是弱引用指针，是一种不控制对象生命周期的智能指针，指向一个 `shared_ptr` 管理的对象。`weak_ptr` 只提供了对管理对象的一种访问手段。


#### 1.22.4.2. 为什么要使用 weak_ptr
