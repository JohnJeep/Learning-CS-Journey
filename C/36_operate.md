...
`hello` 的首地址，即图中的 `ch[0]:104 'h'`。执行
`*p++`时，根据两者的优先级顺序在同一级，从右向左的结合性原则，先执行 `++`，后执行 `*` 操作。`p++`
是先执行当前的操作，然后再执行 `++` 操作。 这条语句执行完成后，指针 p 指向字符串的下一个位置 `ch[1]
101 'e'`。因此我们在终端看到的输出结果为字符 `h`，而程序中 `*p` 的值为 `e`。
