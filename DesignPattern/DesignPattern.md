 ...
 * @LastEditTime: 2025-04-23 17:00:32
...
<font color=red>
适配器模式是一种结构型设计模式，主要用于解决两个接口不兼容的类无法直接协作的问题。它通过引入一个 “适配器” 类，将某个类的接口转换为另一个接口，使得原本因接口不匹配而无法一起工作的类可以协同工作。
</font>

**核心思想**

- **目标**：让不兼容的接口变得兼容，实现 “接口转换”。
...
- 结构
  ...
  - **适配者（Adaptee）**：需要被适配的现有类，其接口与目标接口不兼容。
  - **适配器（Adapter）**：实现目标接口，并委托适配者的方法。
...
- **适配限制**：类适配器依赖继承，可能受限于语言特性（如 Python 支持多重继承，但 Java 不支持）；对象适配器的调用效率可能略低于直接调用。
