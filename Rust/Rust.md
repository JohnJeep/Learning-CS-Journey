...
1. 掌握语法、工具链、生态；
2. 所有权系统（ownership）+ 借用检查器（borrow checker）；
   - **Go**：GC 帮你管内存，你几乎不用想生命周期问题
   - **C++**：你自己管（或用智能指针管），编译器不会拦着你犯错，出问题是运行时的事（悬垂指针、内存泄漏、data race）
   - **Rust**：编译器在**编译期**强制检查所有权和借用规则，你想犯的内存错误，很多时候代码根本编译不过
3. trait、生命周期标注；
...
Rust 目前有 **大约 38 个严格关键字（strict keywords）**，外加一些保留关键字和特殊场景关键字。具体分三类：

## 3.1. 当前使用的关键字（Strict Keywords，35个左右）
...
## 3.2. 保留但未使用的关键字（Reserved Keywords）

这些是为将来扩展预留的，现在不能用作标识符，但目前没有实际功能：
...
## 3.3. 弱关键字（Weak Keywords）

这些只在特定上下文中是关键字，其他地方可以正常当标识符用：

```
macro_rules   union   'static   dyn
```

比如 `union` 只在定义联合体时是关键字：
