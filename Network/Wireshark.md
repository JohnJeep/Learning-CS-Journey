...
Ethereal 是 Wireshark 的前身，2006 年改名为 Wireshark。

# 3. 原理

1.  捕获二进制流量
2.  转换 Wireshark 组装数据包
3.  分析捕获的数据包、识别协议等信息
...
1.  功能：将 MAC 地址、IP 地址、端口号等转换为名字，方便记忆，默认是开启了 MAC 地址解析。包括三种解析器：MAC
    地址解析、网络名字解析、传输名字解析。
2.  缺点：开启 IP 地址或端口解析可以加大可读性，但是也会出现解读错误并且资源消耗大。
