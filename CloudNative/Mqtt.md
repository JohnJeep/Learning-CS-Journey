...
- [1. EMQX](#1-emqx)
  - [1.1. Port](#11-port)
    - [1.1.1. 开放防火墙端口](#111-开放防火墙端口)
    - [1.1.2. 检查防火墙端口是否开放](#112-检查防火墙端口是否开放)
  - [1.2. env](#12-env)
    - [1.2.1. 查找 EMQX 安装目录](#121-查找-emqx-安装目录)
    - [1.2.2. 编辑环境变量](#122-编辑环境变量)
    - [1.2.3. 验证是否生效](#123-验证是否生效)
    - [1.2.4. 系统级环境变量（可选）](#124-系统级环境变量可选)
  - [1.3. ClientID](#13-clientid)
  - [1.4. 介绍](#14-介绍)
  - [1.5. 冲突的 MQTT 客户端 ID](#15-冲突的-mqtt-客户端-id)
  - [1.6. 参数配置](#16-参数配置)
    - [1.6.1. retain](#161-retain)
    - [1.6.2. retainHandling](#162-retainhandling)
- [2. mosquitto](#2-mosquitto)
...
EMQX 使用多个端口来处理不同类型的通信。以下是 EMQX 需要开放的主要端口：
...
Linux Ubuntu 上安装 mosquitto client：
...
Linux Ubuntu 上安装 mosquitto server：
...
若直接启动 mosquitto 服务器，某些系统不能使用 `systemctl` 命令，可以直接运行 mosquitto：
...
mosquitto 服务器默认监听 1883 端口，可以使用以下命令查看：
...
```bash
mosquitto_sub -h <broker_host> -p <port> -t <topic>
```

常用参数说明

- `-h`：MQTT 代理（Broker）的主机地址（默认 `localhost`）
- `-p`：端口（默认 `1883`，TLS 通常为 `8883`）
- `-t`：要订阅的主题（支持通配符 `+` 和 `#`）
- `-u` / `-P`：用户名和密码（如果 Broker 启用了认证）
- `-v`：显示主题名 + 消息内容（verbose 模式）
- `-d`：启用调试输出



示例 1：订阅本地 Broker 的某个主题

```
mosquitto_sub -h localhost -t "sensors/temperature"
```

示例 2：订阅带通配符的主题（接收所有传感器数据）
...
示例 3：连接远程 Broker 并认证

```
mosquitto_sub -h mqtt.example.com -p 1883 -u myuser -P mypass -t "home/livingroom/#" -v
```
...
```bash
mosquitto_pub -h <broker_host> -p <port> -t <topic> -m <message>
```
...
```
mosquitto_pub -h mqtt.example.com -u myuser -P mypass -t "home/light" -m "ON"
```
